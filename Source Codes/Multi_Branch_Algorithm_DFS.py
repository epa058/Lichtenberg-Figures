...
class LeastResistance:
    ...
    def updateOrder(self, grid, row, col):
        ...
        # if there are too many recursions, the algorithm terminates
        elif branch >= 1000:
            return True
            ...
            # keys of minimum entry in coords
            for key in coords:
                if coords[key] == min_value:
                    get_min.append(key)
                    
        for new_coords in get_min:
            new_row, new_col = row + new_coords[0], col + new_coords[1]
            ...
            # recursively calls itself
                ...
                self.updateOrder(grid, new_row, new_col)
